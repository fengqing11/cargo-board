...
const PAGE_SIZE = 50
const AUTO_PAGE_INTERVAL_MS = 8_000
...
function App() {
  ...
  const [autoPage, setAutoPage] = useState(() => readSavedBoolean(AUTO_PAGE_STORAGE_KEY, true))
  ...
  useEffect(() => {
    window.localStorage.setItem(AUTO_PAGE_STORAGE_KEY, String(autoPage))
  }, [autoPage])
  ...
  useEffect(() => {
    if (!autoPage || loading || totalPages <= 1 || previewItem) return
  ...
  }, [autoPage, loading, previewItem, totalPages])
      ...
      <section className="table-card">
        <div className="table-meta">
          ...
          <div className="pagination">
            ...
            <button type="button" className={`secondary auto-page-toggle${autoPage ? ' active' : ''}`} onClick={() => setAutoPage((current) => !current)} disabled={totalPages <= 1}>{t.autoPage}</button>
          </div>
        </div>
